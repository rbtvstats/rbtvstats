angular.module('app.viewer').config(function($stateProvider) {
    $stateProvider.state('viewer.series.all', {
        url: '/',
        templateUrl: 'app/viewer/views/series/series-all/series-all.html',
        controller: function($scope, $state, NgTableParams, InitSrv, StateSrv, SeriesSrv) {
            ...
            $scope.one = function(series) {
                $state.transitionTo('viewer.series.one', { seriesId: series.id });
            };
